...
def fill_mesh(mesh2fill, file, num_aug, scale_verts, flip_edges, slide_verts):
    load_path = get_mesh_path(file, num_aug)
    if os.path.exists(load_path):
        mesh_data = np.load(load_path, encoding='latin1', allow_pickle=True)
    else:
        mesh_data = from_scratch(file, num_aug, scale_verts, flip_edges, slide_verts)
...
def from_scratch(file, num_aug, scale_verts=None, flip_edges=None, slide_verts=None):
    ...
    if num_aug > 1: faces = augmentation(mesh_data, scale_verts=scale_verts, flip_edges=flip_edges, faces=faces)
    build_gemm(mesh_data, faces, face_areas)
    if num_aug > 1: post_augmentation(mesh_data, slide_verts=slide_verts)
...
def augmentation(mesh, scale_verts=None, flip_edges=None, faces=None):
    if scale_verts: scale_verts(mesh)
    if flip_edges: faces = flip_edges(mesh, flip_edges, faces)
    return faces


def post_augmentation(mesh, slide_verts=None):
    if slide_verts: slide_verts(mesh, slide_verts)
