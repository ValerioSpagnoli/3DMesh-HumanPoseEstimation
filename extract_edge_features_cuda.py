...
def meshcnn_like_features(edge_properties):
    ...
    face_0_vertices = cp.array(edge_properties['face_0']['vertices'])
    face_1_vertices = cp.array(edge_properties['face_1']['vertices'])

    normal_0 = cp.cross(face_0_vertices[1] - face_0_vertices[0], face_0_vertices[2] - face_0_vertices[0]).astype(cp.float32)
    normal_0 = normal_0 / cp.linalg.norm(normal_0)
    normal_1 = cp.cross(face_1_vertices[1] - face_1_vertices[0], face_1_vertices[2] - face_1_vertices[0]).astype(cp.float32)
    normal_1 = normal_1 / cp.linalg.norm(normal_1)

    cos_angle = cp.dot(normal_0, normal_1)
    cos_angle = cp.clip(cos_angle, -1, 1)
    dihedral_angle = cp.arccos(cos_angle)
    ...
    for i in range(2):
        ...
        vertex_1 = [v for v in edge_0 if not cp.array_equal(v, common_vertex)][0]
        vertex_2 = [v for v in edge_1 if not cp.array_equal(v, common_vertex)][0]
        d1 = common_vertex - vertex_1
        d1 = d1 / cp.linalg.norm(d1)
        d2 = vertex_2 - common_vertex
        d2 = d2 / cp.linalg.norm(d2)
        cos_angle = cp.clip(cp.dot(d1, d2), -1, 1)
        inner_angle = cp.arccos(cos_angle)
        # FEATURE 2/3: inner angles
        edge_features[f'inner_angle_{i}'] = inner_angle

    #* Compute the length-height ratio  
    edge_length = cp.linalg.norm(edge_properties['vertices'][0] - edge_properties['vertices'][1])
    for i in range(2):
        ...
        face_area = cp.linalg.norm(cp.cross(face_vertex_1 - face_vertex_0, face_vertex_2 - face_vertex_0))
...
def simple_features(edge_properties):
    ...
    return cp.array([vertex_0[0], vertex_0[1], vertex_0[2], vertex_1[0], vertex_1[1], vertex_1[2], edge_length])
  
  
def simple_features_2d(edge_properties):
    edge_vertex_0 = cp.array(edge_properties['vertices'][0])
    edge_vertex_1 = cp.array(edge_properties['vertices'][1])
    edge_length = cp.linalg.norm(edge_vertex_0 - edge_vertex_1)

    edge_features = cp.array([edge_vertex_0[0], edge_vertex_0[1], edge_vertex_0[2], 
                              edge_vertex_1[0], edge_vertex_1[1], edge_vertex_1[2], 
                              edge_length])

    f0_n0_features = cp.zeros(7)
    f0_n1_features = cp.zeros(7)
    f1_n0_features = cp.zeros(7)
    f1_n1_features = cp.zeros(7)

    for i in range(2):
        neighbor_0 = edge_properties[f'face_{i}']['neighbor_0']['vertices']
        neighbor_0_vertex_0 = cp.array(neighbor_0[0])
        neighbor_0_vertex_1 = cp.array(neighbor_0[1])
        neighbor_0_length = cp.linalg.norm(neighbor_0_vertex_0 - neighbor_0_vertex_1)

        neighbor_0_features = cp.array([neighbor_0_vertex_0[0], neighbor_0_vertex_0[1], neighbor_0_vertex_0[2],
                                        neighbor_0_vertex_1[0], neighbor_0_vertex_1[1], neighbor_0_vertex_1[2], 
                                        neighbor_0_length])
        if i == 0:
            f0_n0_features = neighbor_0_features
        elif i == 1:
            f1_n0_features = neighbor_0_features

        neighbor_1 = edge_properties[f'face_{i}']['neighbor_1']['vertices']
        neighbor_1_vertex_0 = cp.array(neighbor_1[0])
        neighbor_1_vertex_1 = cp.array(neighbor_1[1])
        neighbor_1_length = cp.linalg.norm(neighbor_1_vertex_0 - neighbor_1_vertex_1)

        neighbor_1_features = cp.array([neighbor_1_vertex_0[0], neighbor_1_vertex_0[1], neighbor_1_vertex_0[2],
                                        neighbor_1_vertex_1[0], neighbor_1_vertex_1[1], neighbor_1_vertex_1[2], 
                                        neighbor_1_length])
        if i == 0:
            f0_n1_features = neighbor_1_features
        elif i == 1:
            f1_n1_features = neighbor_1_features

    return cp.array([edge_features, f0_n0_features, f0_n1_features, f1_n0_features, f1_n1_features])


def compute_edges_features(edges_properties, features_type):
    if features_type == 'meshcnn_like_features':
        edges_features = cp.array([[d['dihaedral_angle'], d['inner_angle_1'], d['inner_angle_2'], d['length_height_ratio_1'], d['length_height_ratio_2']] for d in edges_properties])
    elif features_type == 'simple_features':
        edges_features = cp.array([simple_features(d) for d in edges_properties])
    elif features_type == 'simple_features_2d':
        edges_features = cp.zeros((1, 5, 7))
        for edge_properties in edges_properties:
            edge_features = simple_features_2d(edge_properties)
            edge_features = cp.expand_dims(edge_features, axis=0)
            edges_features = cp.vstack((edges_features, edge_features))
        ...
        if os.path.isdir(subfolder_path):
            ...
            for mesh_file in tqdm(mesh_files, desc=f'Processing meshes in {subfolder}'):
                if mesh_file.endswith('.obj'):
                    ...
                    edges_features_tensor = torch.tensor(cp.asnumpy(edges_features))
